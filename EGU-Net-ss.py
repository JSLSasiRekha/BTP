...
def initialize_parameters():
    ...
    x_w1 = tf.get_variable("x_w1", [1,1,224,256], dtype=tf.float32, initializer=initializer)
    x_b1 = tf.get_variable("x_b1", [256], initializer = tf.zeros_initializer())
     
    x_w2 = tf.get_variable("x_w2", [1,1,256,128], dtype=tf.float32, initializer=initializer)
    x_b2 = tf.get_variable("x_b2", [128], initializer = tf.zeros_initializer())   
    
    x1_conv_w1 = tf.get_variable("x1_conv_w1", [5,5,224,256], dtype=tf.float32, initializer=initializer)
    x1_conv_b1 = tf.get_variable("x1_conv_b1", [256], initializer = tf.zeros_initializer())
    
    x1_conv_w2 = tf.get_variable("x1_conv_w2", [3,3,256,128], dtype=tf.float32, initializer=initializer)
    x1_conv_b2 = tf.get_variable("x1_conv_b2", [128], initializer = tf.zeros_initializer()) 

    x_w3 = tf.get_variable("x_w3", [1,1,128,32], dtype=tf.float32, initializer=initializer)
    x_b3 = tf.get_variable("x_b3", [32], initializer = tf.zeros_initializer())
    
    x1_conv_w4 = tf.get_variable("x1_conv_w4", [1,1,5,32], dtype=tf.float32, initializer=initializer)
    x1_conv_b4 = tf.get_variable("x1_conv_b4", [5], initializer = tf.zeros_initializer())

    x_w4 = tf.get_variable("x_w4", [1,1,32,5], dtype=tf.float32, initializer=initializer)
    x_b4 = tf.get_variable("x_b4", [5], initializer = tf.zeros_initializer())
    
    x_dew1 = tf.get_variable("x_dew1", [1,1,32,5], dtype=tf.float32, initializer=initializer)
    x_deb1 = tf.get_variable("x_deb1", [32], initializer = tf.zeros_initializer())
    
    x_dew2 = tf.get_variable("x_dew2", [1,1,128,32], dtype=tf.float32, initializer=initializer)
    x_deb2 = tf.get_variable("x_deb2", [128], initializer = tf.zeros_initializer())
 
    x_dew3 = tf.get_variable("x_dew3", [3,3,256,128], dtype=tf.float32, initializer=initializer)
    x_deb3 = tf.get_variable("x_deb3", [256], initializer = tf.zeros_initializer())
    
    x_dew4 = tf.get_variable("x_dew4", [5,5,224,256], dtype=tf.float32, initializer=initializer)
...
def train_mynetwork(x_pure_set, x_mixed_set, x_mixed_set1, y_train, y_test, learning_rate_base = 0.1, beta_reg = 0.005, num_epochs = 200, minibatch_size = 8000):
...
Pure_TrSet = scio.loadmat('TNNLS_Data/Data/Pure_TrSet.mat')
Mixed_TrSet = scio.loadmat('TNNLS_Data/Data/Mixed_TrSet.mat')

TrLabel = scio.loadmat('TNNLS_Data/Data/TrLabel.mat')
TeLabel = scio.loadmat('TNNLS_Data/Data/TeLabel.mat')
